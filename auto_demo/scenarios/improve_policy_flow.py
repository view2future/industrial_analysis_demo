#!/usr/bin/env python3
"""
Script to improve the policy analysis flow with better waits and element checking
"""
...
def improve_policy_analysis_flow():
  ...
  - action: wait
    duration: 6
    description: "等待政策分析页面基本加载"
    subtitle: "正在加载政策分析工作台界面"

  # Wait for the main tab container to be visible
  - action: wait
    selector: "#policyTabs"
    duration: 10
    description: "等待政策分析页面完全加载"
  ...
  - action: wait
    selector: "#searchPoliciesBtn"
    duration: 10
    description: "等待开始检索按钮变为可用"
    subtitle: "检索功能完全就绪，开始检索按钮已激活"
  ...
  - action: click
    ...
    print("Successfully updated comprehensive_demo.yaml with improved waits for policy analysis!")
